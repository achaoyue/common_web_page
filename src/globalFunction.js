...
const globalFunction = {
    ...
    simpleNumberToChinese: function (num) {
        const chineseNums = ['零', '一', '二', '三', '四', '五', '六', '七', '八', '九'];
        const chineseUnits = ['', '十', '百', '千'];
        let numStr = num.toString();
        let pIdx = 0;
        let r = "";
        for (let i = numStr.length - 1; i >= 0; i--, pIdx++) {


            if (numStr[i] == '0' && numStr[i + 1] == '0' && (numStr.length - i - 1) % 4 != 0) {
                continue;
            }

            r = (((numStr.length - i - 1) % 4 == 0 && numStr[i] == '0') ? '' : chineseNums[Number(numStr[i])]) + ((numStr.length - i - 1) % 4 != 0 && numStr[i] == '0' ? '' : chineseUnits[pIdx]) + r;
    ...
    pointToChinese: function (num) {
        if (num == '') {
            return "";
        }
        const chineseNums = ['零', '一', '二', '三', '四', '五', '六', '七', '八', '九'];

        return "点" + num.toString().replace(/\d/g, match => {
    ...
    numberToChinese: function (num) {
        ...
        let pointNum = num.toString().split(".")[1];
        let pointStr = "";
        if (pointNum !== '' && pointNum != undefined) {
            pointStr = this.pointToChinese(Number(pointNum))
        }
        ...
        let slot = [];
        //四位切割
        for (let i = num; i > 0; i = Math.floor(i / 10000)) {
            slot.push(i % 10000);
        }
        ...
        for (let i = 0; i < slot.length; i++) {
            let r = this.simpleNumberToChinese(slot[i])

            if (slot[i] !== 0) {
                r = r + sp[i];
            }

            if (i > 0 && slot[i - 1] < 1000 && slot[i - 1] !== 0) {
                result = "零" + result;
            }
